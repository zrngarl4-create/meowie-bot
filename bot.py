...
def _extract_bold_metadata(text):
    """
    دنبال بخش‌های **این‌شکلی** تو متن می‌گرده، خودِ دو ستاره‌ها رو از متن
    حذف می‌کنه و برای هر بخش، بازه‌ی Bold رو بر حسب واحد UTF-16 (چیزی که
    خودِ روبیکا برای from_index/length می‌خواد، نه len() ساده‌ی پایتون)
    حساب می‌کنه.
    خروجی: (متنِ پاک‌شده از **, لیستِ meta_data_parts)
    اگه هیچ ** ای تو متن نباشه، لیست خالی برمی‌گرده و متن دست‌نخورده می‌مونه.
    """
    parts = []
    clean_chunks = []
    last_end = 0
    ...
    for m in BOLD_PATTERN.finditer(text):
        ...
        clean_chunks.append(before)
        utf16_offset += _utf16_length(before)

        bold_text = m.group(1)
        clean_chunks.append(bold_text)
        bold_len = _utf16_length(bold_text)
        if bold_len:
            parts.append({
                "type": "Bold",
                "from_index": utf16_offset,
                "length": bold_len,
            })
        utf16_offset += bold_len
        last_end = m.end()

    clean_chunks.append(text[last_end:])
    return "".join(clean_chunks), parts


def send_message(chat_id, text, reply_to_message_id=None):
    # اگه تو متن **...** باشه، اول با متادیتای Bold امتحان می‌کنیم. اگه
    # روبیکا رد کرد (یا هر خطای دیگه‌ای داد)، کل بدنه‌ی پاسخ رو لاگ
    # می‌کنیم (نه فقط کد وضعیتش) تا واقعاً معلوم بشه ایراد از کجاست، و
    # بعد بدون ** به‌صورت متن ساده می‌فرستیم که پیام گم نشه.
    clean_text, bold_parts = _extract_bold_metadata(text)
    ...
    if bold_parts:
        payload_with_bold = dict(payload)
        payload_with_bold["metadata"] = {"meta_data_parts": bold_parts}
        try:
            resp = requests.post(f"{BASE_URL}/sendMessage", json=payload_with_bold, timeout=15)
            if resp.status_code == 200:
                return
            print("بولد رد شد، می‌ریم سراغ متن ساده. پاسخ کامل روبیکا:",
                  resp.status_code, resp.text)
        except Exception as e:
            print("خطا در تلاش ارسال بولد:", e)

    try:
        resp = requests.post(f"{BASE_URL}/sendMessage", json=payload, timeout=15)
        if resp.status_code != 200:
            print("خطای HTTP در ارسال پیام:", resp.status_code, resp.text)
    except Exception as e:
...
def handle_reset_name(chat_id, sender_id, message_id, reply_to_message_id):
    if str(sender_id) not in ADMIN_USER_IDS:
        send_message(chat_id, "⛔️ این قابلیت فقط برای مدیرهاست.", reply_to_message_id=message_id)
        return
    ...
    send_message(chat_id, "✅ اسم این کاربر پاک شد؛ دفعه‌ی بعد باید دوباره برای خودش اسم بذاره.", reply_to_message_id=message_id)
...
def build_shop_message():
    ...
    if not items:
        return "🏪 فروشگاه فعلاً خالیه، بعداً دوباره سر بزن."

    lines = ["🏪 فروشگاه PawKing", ""]
...
def handle_use_item(chat_id, sender_id, message_id, text):
...
def _dispatch_message(chat_id, sender_id, message_id, text, reply_to_message_id):
    ...
    # دستور ریست نام: فقط برای ادمین‌ها، روی ریپلای
    ...
    if text == "میو":
        ...
        handle_use_item(chat_id, sender_id, message_id, text)
...
def send_daily_gift_code_to_admins(code):
    text = (
        "🎁 کد هدیه‌ی امروز:\n\n"
        f"«{code}»\n\n"
        f"💰 هر نفر با فرستادن «{GIFT_CODE_PREFIX} {code}» تو گروه، {format_number(db.GIFT_CODE_COIN_VALUE)} 🪙 می‌گیره.\n"
        f"👥 فقط {db.GIFT_CODE_MAX_USES} نفر اول می‌تونن استفاده‌ش کنن — می‌تونی همین الان تو یه گروه بگذاریش."
    )
    for admin_id in ADMIN_USER_IDS:
        try:
            send_message(admin_id, text)
        except Exception:
            print(f"نتونستم کد هدیه رو به ادمین {admin_id} پیوی بدم:")
            traceback.print_exc()
...
def _run_maintenance_tick(last_cleanup, last_broadcast_at):
    ...
    # می‌سازیم و پیوی ادمین‌ها می‌فرستیم (فقط یه‌بار در روز)
    try:
        if db.iran_now().hour >= 12:
            code = db.generate_daily_gift_code_if_needed()
            if code:
                send_daily_gift_code_to_admins(code)
    except Exception:
        print("خطا در ارسال کد هدیه‌ی روزانه:")
        traceback.print_exc()
