...
def send_message_with_mention(chat_id, text, mention_name, mention_user_id, reply_to_message_id=None):
    """
    مثل send_message (کل متن بولد می‌شه)، با این تفاوت که اگه mention_name
    تو متن پیدا بشه، همون تیکه رو به‌صورت یه mention آبی/لینک‌دار به آیدی
    داخلی (mention_user_id) کاربر می‌فرسته — یعنی هر کی روش بزنه، اکانت
    همون کاربر باز می‌شه.

    نکته‌ی مهم: این نوع metadata برخلاف Bold، جایی تو مستندات رسمی
    روبیکا برای بات‌ها تأیید نشده و کاملاً آزمایشیه. برای همین، دقیقاً
    طبق همون الگوی دفاعی send_message: اگه روبیکا این metadata رو رد
    کنه (چه با خطای HTTP، چه با status خطا تو بدنه‌ی پاسخ)، به‌جای گم
    شدن پیام، بی‌صدا به send_message عادی (بولد ساده، بدون mention)
    برمی‌گردیم.

    نسخه‌ی دوم: type رو از "MentionText" به "Mention" و فیلد آیدی رو
    از "mention_text_object_guid" به "user_id" تغییر دادیم (هماهنگ با
    نام‌گذاری خودِ API روبیکا تو بقیه‌ی جاها)، چون نسخه‌ی قبلی یا رد
    می‌شد یا رنگ‌بندیش رو بخش اشتباهی از متن می‌افتاد.
    """
    idx = text.find(mention_name) if mention_name else -1
    if idx == -1:
        # اسم تو متن پیدا نشد (یا اصلاً پاس داده نشده)؛ چیزی برای لینک
        # کردن نیست، همون حالت عادی رو بفرست.
        return send_message(chat_id, text, reply_to_message_id=reply_to_message_id)

    prefix_len = _utf16_length(text[:idx])
    name_len = _utf16_length(mention_name)
    ...
    try:
        resp = requests.post(f"{BASE_URL}/sendMessage", json=payload, timeout=15)
        if resp.status_code != 200:
            print("خطای HTTP در ارسال پیام با mention، برگشت به حالت عادی:", resp.status_code, resp.text)
            raise RuntimeError("send with mention metadata failed")
        try:
            data = resp.json()
        except ValueError:
            data = None
        if isinstance(data, dict) and data.get("status") not in (None, "OK", "ok", "done", "Done"):
            print("خطای منطقی سرور در ارسال پیام با mention، برگشت به حالت عادی:", data)
            raise RuntimeError("send with mention metadata returned error status")
        return True
    except Exception as e:
        print("ارسال با mention شکست خورد، تلاش دوباره بدون لینک:", e)
