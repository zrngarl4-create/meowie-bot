...
def handle_cat_set_name(chat_id, sender_id, message_id, text):
    new_name = text[len(CAT_NAME_PREFIX):].strip()
    if not new_name:
        send_message(
            chat_id,
            "بعد از «تنظیم پیشی» اسمشو بنویس، مثلاً:\nتنظیم پیشی هانیه",
            reply_to_message_id=message_id,
        )
        return
    ...
    if updated:
        send_message(chat_id, f"✅ اسم پیشیت شد: {new_name} 🐾", reply_to_message_id=message_id)
    else:
        send_message(
            chat_id,
            "هنوز پیشی نداری! اول بنویس «پیشی» تا یکی به سرپرستی بگیری 🐾",
            reply_to_message_id=message_id,
        )
...
def handle_hand_word(chat_id, sender_id, message_id, text):
    ...
    if is_private:
        ...
        if not ok:
            return  # تو بازی‌ای نیست که نیاز به قایم‌کردن داشته باشه؛ نادیده می‌گیریم
        group_chat_id = result["chat_id"]
        send_message(sender_id, "✅ گل قایم شد! حالا صبر کن طرف مقابل حدس بزنه.")
        send_message(
            group_chat_id,
        ...
        if not ok:
            return  # تو بازی‌ای نیست که نوبت حدس زدنش باشه؛ نادیده می‌گیریم
...
def _dispatch_message(chat_id, sender_id, message_id, text, reply_to_message_id):
    ...
    elif text.startswith(CAT_NAME_PREFIX):
