...
def send_message(chat_id, text, reply_to_message_id=None):
    # قبلاً اینجا اول با یه متادیتای Bold تلاش می‌کرد که همیشه (بدون
    # استثنا) با INVALID_ACCESS رد می‌شد، و فقط بعد از شکست دوباره ساده
    # می‌فرستاد. یعنی هر پیام بات دو بار درخواست به روبیکا می‌زد — یکی
    # که همیشه شکست می‌خورد (فقط وقت تلف می‌کرد و لاگ رو پر می‌کرد)، بعد
    # یکی که واقعاً جواب می‌داد. چون تلاش بولد ۱۰۰٪ شکست می‌خورد، حذفش
    # کردیم و مستقیم می‌ریم سراغ همون روشی که واقعاً کار می‌کنه.
