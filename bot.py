...
def send_message_with_mention(chat_id, text, mention_name, mention_user_id, reply_to_message_id=None):
    ...
    نکته‌ی مهم: این نوع metadata (نوع MentionText) برخلاف Bold، جایی تو
    مستندات رسمی روبیکا برای بات‌ها تأیید نشده و کاملاً آزمایشیه. برای
    همین، دقیقاً طبق همون الگوی دفاعی send_message: اگه روبیکا این
    metadata رو رد کنه (چه با خطای HTTP، چه با status خطا تو بدنه‌ی
    پاسخ)، به‌جای گم شدن پیام، بی‌صدا به send_message عادی (بولد ساده،
    بدون mention) برمی‌گردیم.
    """
    ...
    metadata = {
        "meta_data_parts": [
            {"from_index": 0, "length": _utf16_length(text), "type": "Bold"},
            {
                ...
                "length": name_len,
                "type": "MentionText",
                "mention_text_object_guid": mention_user_id,
