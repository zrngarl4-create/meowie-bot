...
def handle_word_guess_start(chat_id, sender_id, message_id):
    ...
    if not ok:
        ...
        if reason == "already_active":
            ...
            send_message(chat_id, "بانک کلمه خالیه، بعداً دوباره امتحان کن.", reply_to_message_id=message_id)
        return
...
def handle_word_guess_submit(chat_id, sender_id, message_id, text):
    ...
    if result["solved"]:
        send_message(
            chat_id,
            (
                f"🎉 آفرین! کلمه «{result['word']}» بود و درست حدسش زدی!\n"
    ...
    if result["finished"]:
        send_message(
            chat_id,
            f"😿 تلاش‌هات تموم شد. کلمه‌ی درست «{result['word']}» بود.",
            reply_to_message_id=message_id,
        )
        return
    ...
    send_message(
        ...
        (
            f"❌ غلط بود.\n\n"
            f"راهنما: {display}\n"
            f"{attempts_left} تلاش دیگه داری."
        ),
        reply_to_message_id=message_id,
    )
