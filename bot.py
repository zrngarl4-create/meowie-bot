...
def build_profile_message(display_name, user_id, profile):
    ...
    name_line = f"👑 {display_name}" if cosmetics["has_crown"] else display_name
...
def handle_force_shop_cycle(chat_id, sender_id, message_id):
    if str(sender_id) not in ADMIN_USER_IDS:
        send_message(chat_id, "⛔️ این قابلیت فقط برای مدیرهاست.", reply_to_message_id=message_id)
        return
    new_cycle = db.force_rotate_shop_cycle()
    if not new_cycle:
        send_message(chat_id, "چرخه‌ای برای رفتن بهش پیدا نشد.", reply_to_message_id=message_id)
        return
    send_message(chat_id, f"🔄 فروشگاه چرخید به: {new_cycle['name']}", reply_to_message_id=message_id)
...
def handle_admin_grant_item(chat_id, sender_id, message_id, text, reply_to_message_id):
    ...
    if not reply_to_message_id:
        send_message(
            chat_id,
            f"باید روی پیام همون کاربر ریپلای بزنی و بنویسی:\n{GRANT_ITEM_PREFIX} <کلیدواژه آیتم>\nمثلاً: {GRANT_ITEM_PREFIX} سپرشرکت",
            reply_to_message_id=message_id,
        )
        return
    ...
    keyword = text[len(GRANT_ITEM_PREFIX):].strip()
    if not keyword:
        send_message(
            chat_id,
            f"بعد از «{GRANT_ITEM_PREFIX}» کلیدواژه‌ی آیتم رو بنویس، مثلاً:\n{GRANT_ITEM_PREFIX} سپرشرکت",
            reply_to_message_id=message_id,
        )
        return
...
def handle_admin_help(chat_id, sender_id, message_id):
    ...
    msg = (
        ...
        "└ چرخوندن دستی چرخه‌ی فروشگاه (برای تست، بدون توجه به زمان‌بندی)\n\n"
...
def _dispatch_message(chat_id, sender_id, message_id, text, reply_to_message_id):
    ...
    if text == SHOP_CYCLE_FORCE_COMMAND:
        handle_force_shop_cycle(chat_id, sender_id, message_id)
        return
